...
这个目录保存仓库较早期使用的预验证脚本，主要用于零样本 / 少量样本快速试跑，不属于当前主线训练流水线。
...
- `preview_chatml.py`
  预览输入样本，检查 ChatML 格式和 prompt 拼接。
- `prompt.txt`
  旧版预验证使用的 prompt。
- `README.md`
  当前目录说明。
...
```bash
.venv/bin/python src/prevalidation/run_pretest_vllm.py \
...
```bash
.venv/bin/python src/prevalidation/summarize_pretest.py \
