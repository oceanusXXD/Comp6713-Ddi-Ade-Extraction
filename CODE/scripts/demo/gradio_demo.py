"""Gradio demo for direct text inference.

This demo reuses the same lower-level inference stack as
`scripts/inference/predict.py`, but does not invoke the CLI script itself and
does not require a dataset file for free-form text input.
"""
...
def build_example_catalog(limit: int = 128) -> List[Dict[str, Any]]:
    """Build optional quick-fill examples.

    Missing example files are allowed. In that case the demo returns an empty
    catalog and still starts in free-input mode.
    """
    candidates = [
...
def resolve_adapter_for_demo(adapter_path: Optional[Path]) -> Tuple[Optional[Path], str]:
    ...
    Preference order:
    1. Configured adapter path as-is.
    2. If config points at `.../final_adapter` but only the run directory exists, search that run directory.
    3. A PEFT adapter directly under the run directory.
    4. The newest `checkpoint-*` adapter under the run directory.
    """
...
def run_one_inference(
    ...
    if task_mode == EXTRACTION_MODE and isinstance(example_value, int) and example_value >= 0 and example_value < len(catalog):
