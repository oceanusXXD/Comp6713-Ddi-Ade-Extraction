"""Main repository inference entrypoint.

Supported modes:
1. Run batch inference on a JSONL dataset and optionally compute metrics.
2. Run single-sample inference on one raw text input.
"""
...
def set_global_seed(seed: int, *, seed_cuda: bool = True) -> None:
    """Set random seeds for inference."""
    random.seed(seed)
...
def build_single_example(
    ...
    *,
    ...
    """Wrap one raw text input into the same sample object used for batch inference."""
...
def _preview_text(text: str, *, limit: int = 240) -> str:
    """Flatten multiline text into a short logging preview."""
...
def log_effective_config(config: Dict[str, Any], args: argparse.Namespace) -> None:
    """Log the resolved inference config for debugging and traceability."""
...
def write_jsonl(path: Path, rows: Sequence[Dict[str, Any]]) -> None:
    """Write prediction rows to JSONL."""
...
def write_metrics_if_available(rows: Sequence[Dict[str, Any]], config: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """Compute and write metrics when gold relations are present."""
    if not rows:
        return None
...
def summarize_prediction_rows(rows: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
    """Summarize parse status and non-empty prediction rate."""
    total_rows = len(rows)
    parsed_rows = 0
    nonempty_rows = 0
    relation_total = 0
...
def log_prediction_debug_summary(rows: Sequence[Dict[str, Any]]) -> None:
    """Log prediction summaries and extra detail for single or failed samples."""
...
def main() -> None:
    """Main entrypoint for repository inference."""
    args = parse_args()
    ...
    try:
        if args.input_text is not None:
            # Single-text mode does not depend on a dataset file.
            ...
            # Dataset mode loads gold_relations so evaluation can run immediately after inference.
