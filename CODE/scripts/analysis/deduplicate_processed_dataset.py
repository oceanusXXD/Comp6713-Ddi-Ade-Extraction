"""Deduplicate processed ADE/DDI ChatML datasets.

This script handles duplicate-related cleanup only:

1. Rewrite each row into the canonical training format.
2. Remove exact text overlap between train and validation/test, and between validation and test.
3. Collapse exact duplicate texts within each split.
4. Keep the majority relation set when duplicate texts disagree on labels.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Deduplicate processed ChatML train/validation/test files.")
    parser.add_argument(
        "--input-dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional stats output path. Defaults to <output-dir>/dedup_stats.json.",
    )
...
def canonical_text(text: str) -> str:
    """Build the canonical text key used for deduplication."""
    return normalize_text(text).casefold()


def build_entry(row: Dict[str, Any], *, line_no: int) -> Dict[str, Any]:
    """Convert one raw row into the structure used for deduplication."""
...
def load_entries(path: Path) -> List[Dict[str, Any]]:
    """Load and parse all samples from one split."""
...
def exact_overlap(texts_a: Iterable[str], texts_b: Iterable[str]) -> List[str]:
    """Return the exact overlap between two text collections."""
...
def deduplicate_entries(entries: Sequence[Dict[str, Any]]) -> tuple[List[Dict[str, Any]], Dict[str, Any]]:
    """Collapse duplicate texts within one split."""
...
def count_duplicate_text_rows(rows: Sequence[Dict[str, Any]]) -> int:
    """Count duplicate text rows still remaining within one split."""
    counter = Counter(
...
def main() -> None:
    ...
    notes.extend(
        [
            "Removed exact text overlap between train and test",
            "Removed exact text overlap between validation and test",
            "Collapsed exact duplicate texts within each split",
            "Kept the majority relation set when duplicate texts disagreed",
            "Rewrote assistant targets into the canonical serialized relation format",
        ]
    )
