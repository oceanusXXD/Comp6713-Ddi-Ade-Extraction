"""Helper script for evaluation grouped by augmentation type."""
...
def default_output_path(path: Path, suffix: str) -> Path:
    """Generate a default output filename from an input path."""
    return path.with_name(f"{path.stem}{suffix}")


def main() -> None:
    """Compute prediction metrics grouped by augmentation_type."""
    args = parse_args()
