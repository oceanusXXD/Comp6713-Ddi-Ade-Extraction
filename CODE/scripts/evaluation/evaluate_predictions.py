...
"""General prediction evaluation script.

It evaluates prediction JSONL files from the main inference script, supports
selected legacy formats, and can attach gold relations from a dataset file when
needed.
"""
...
def default_output_path(predictions_path: Path, suffix: str) -> Path:
    """Generate a default output filename from the prediction path."""
...
def attach_gold_labels(
    ...
    *,
    ...
    """Attach gold labels from a dataset when prediction rows do not include them."""
...
def main() -> None:
    """Main entrypoint for prediction evaluation."""
    args = parse_args()
