"""Main repository training entrypoint.

This script ties together the full training flow: config loading, tokenizer and
model loading, dataset statistics, dry-run encoding checks, Trainer setup,
training, and final artifact export.
"""
...
from src.runtime_env import warn_if_not_running_in_virtualenv

LOGGER = logging.getLogger(__name__)


def unwrap_module(model):
    """Return the innermost model from a possibly wrapped module."""
    candidate = model
...
class WeightedSamplingTrainer(Trainer):
    """Extend Hugging Face Trainer with weighted sampling and custom loss logic.

    1. Weighted sampling by per-sample weights.
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        """Override default loss computation to support sample-level loss weighting."""
        ...
        # Use the standard causal-LM one-token-right shift for supervision.
        ...
        if self.use_sample_loss_weights and loss_weights is not None:
            # Aggregate loss with sample weights to reduce dilution by easier examples.
    ...
    def create_optimizer(self):
        """Add support for LoRA+ and LoRA-FA adapter optimizers."""
...
def set_global_seed(seed: int) -> None:
    """Set Python and Torch random seeds."""
    random.seed(seed)
...
def resolve_resume_path(resume_from_checkpoint: Optional[str]) -> Optional[Path]:
    """Resolve the resume path to an absolute path."""
...
def build_training_arguments(config):
    """Map repository training config fields to `TrainingArguments`."""
...
def log_dataset_summary(config) -> None:
    """Log the basic train and validation dataset summaries."""
...
def save_dataset_statistics(config, tokenizer, system_prompt: str) -> None:
    """Write train and validation dataset statistics to the observability directory."""
...
def save_run_configuration(config) -> None:
    """Record runtime-environment and training-config snapshots."""
    output_dir = observability_dir(config)
...
def main() -> None:
    ...
    warn_if_not_running_in_virtualenv(
        ...
        script_name="scripts/train/train_finetune.py",
    )
    ...
    # Encode train and validation rows into the structures consumed directly by Trainer.
    ...
    # Load the full model only after the dry run succeeds.
