"""Batch experiment script for LoRA variants."""
...
def run_command(command: List[str], log_path: Path) -> None:
    """Run an external command and write its combined log to a file."""
...
def materialize_runtime_config(config: Dict[str, Any], path: Path) -> Path:
    """Write the runtime config to disk for reproducibility."""
...
def metric_cell(metrics: Dict[str, Any], key: str) -> float:
    """Extract a metric value from the common metrics structure."""
    if key == "micro_f1":
...
def save_summary(summary: Dict[str, Any], output_json: Path, output_md: Path) -> None:
    """Write both JSON and Markdown benchmark summaries."""
...
def main() -> None:
    """Run LoRA variant experiments in batch from config."""
    args = parse_args()
    ...
    warn_if_not_running_in_virtualenv(
        ...
        script_name="scripts/experiments/run_qwen3_lora_variant_benchmark.py",
    )
    ...
    for variant in args.variants:
        ...
        # This benchmark compares only final adapters, so training-time eval and checkpointing are disabled.
