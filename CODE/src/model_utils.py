"""Training config, model loading, and PEFT wrapper helpers.

This module normalizes training configs, resolves model and tokenizer sources,
handles quantization settings, and applies LoRA or QLoRA adapters when needed.
"""
...
def try_resolve_existing_path(value: str) -> Optional[Path]:
    """Resolve a user-provided string to an existing local path when possible."""
    ...
    if raw_path.is_absolute():
        ...
        # Keep a few compatibility fallbacks for common repository-relative layouts.
        candidates.append((PROJECT_ROOT / "models" / raw_path.name).resolve())
        # Also accept explicit repository-relative inputs such as models/Qwen3-8B.
...
def _default_hf_id_if_local_qwen3_missing(normalized: str) -> Optional[str]:
    """Return the official HF id when the default local Qwen3 path is missing."""
...
def resolve_model_source(value: Optional[str], *, allow_remote: bool = False) -> Optional[str]:
    ...
    Local paths are required by default. Remote identifiers are kept only when
    `allow_remote_model_source` is enabled.
    """
    if value is None:
        return None
...
def validate_local_model_source(value: Optional[str], label: str, *, allow_remote: bool = False) -> None:
    """Validate that a model source is allowed for the current settings."""
    if value is None:
        return
    if allow_remote:
        return
...
def torch_dtype_from_name(dtype_name: Optional[str]) -> Optional[torch.dtype]:
    """Convert a dtype name from config into `torch.dtype`."""
...
def normalize_report_to(value: Any) -> Any:
    """Normalize `report_to` into the list format expected by transformers."""
    if value is None:
        return []
...
def normalize_init_lora_weights(value: Any) -> Any:
    """Normalize LoRA initialization config from booleans or strings."""
    if value is None:
        return True
    if isinstance(value, str):
        lowered = value.strip().lower()
...
def flatten_legacy_sections(loaded: Dict[str, Any]) -> Dict[str, Any]:
    """Flatten legacy sectioned configs into the current field layout.

    This keeps compatibility with earlier model/data/training sections, older
    `peft` blocks, and early qlora/lora switches.
    """
...
def load_training_config(config_path: str | Path) -> Dict[str, Any]:
    """Load a YAML training config, apply defaults, resolve paths, and validate it."""
...
def validate_training_config(config: Dict[str, Any]) -> None:
    """Validate training config values before training starts."""
    ...
    # Guard against combinations that are individually valid but incompatible together.
...
def enforce_hf_offline_mode(*, allow_remote: bool) -> None:
    """Force Hugging Face into offline mode when remote sources are disabled."""
    if allow_remote:
        return
...
def suppress_local_flash_attn_shadowing(*, enabled: bool):
    """Temporarily remove the repository root from `sys.path` to avoid local shadowing.

    This is only used when a real third-party implementation such as
...
def build_quantization_config(config: Dict[str, Any]):
    """Build a bitsandbytes quantization config for QLoRA mode."""
...
def load_model(config: Dict[str, Any]):
    """Load the base causal LM and apply configured dtype, quantization, and attention settings."""
    enforce_hf_offline_mode(allow_remote=bool(config.get("allow_remote_model_source", False)))
    ...
    # Some environments let the local `flash_attn` shim shadow the real package.
...
def apply_peft_if_requested(model: Any, config: Dict[str, Any]) -> Any:
    """Apply LoRA or QLoRA wrappers when requested by config."""
...
def build_model_and_tokenizer(config: Dict[str, Any], tokenizer: Any = None):
    """Build the model and tokenizer used for training."""
...
def save_adapter_artifacts(model: Any, tokenizer: Any, output_dir: Path) -> Path:
    """Write final adapter and tokenizer artifacts to `final_adapter/`."""
