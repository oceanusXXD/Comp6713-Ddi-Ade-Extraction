"""Legacy prevalidation script using the vLLM backend."""
...
def extract_user_text(messages: List[Dict[str, Any]]) -> str:
    """Extract the user text from ChatML messages."""
    for msg in messages:
...
def extract_gold_relations(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Extract gold relations from the assistant ChatML message."""
    for msg in messages:
...
def extract_json_list(text: str) -> Optional[List[Dict[str, Any]]]:
    """Try to recover a JSON list from output text."""
    text = text.strip()
    # Remove any <think>...</think> block before parsing JSON.
...
def main() -> None:
    """Main entrypoint for vLLM prevalidation."""
