"""Data loading, label normalization, and supervised training example helpers.

This module loads ChatML JSONL rows, normalizes assistant outputs into a common
relation format, computes dataset statistics, and encodes supervised examples
into token and label tensors.
"""
...
def normalize_text(value: Any) -> str:
    """Apply lightweight text normalization by collapsing extra whitespace."""
    return " ".join(str(value).strip().split())


def normalize_label_key(raw_label: Any) -> str:
    """Normalize labels into a lookup-friendly key format."""
    label = normalize_text(raw_label)
    return re.sub(r"[-_\s]+", "-", label).upper()


def normalize_label(raw_label: Any) -> str:
    """Map historical label aliases to the repository canonical labels."""
...
def parse_assistant_relations(assistant_content: str) -> List[Dict[str, str]]:
    """Parse assistant JSON output and normalize fields and labels."""
...
def normalize_relation_list(relations: Sequence[Dict[str, str]]) -> List[Dict[str, str]]:
    """Deduplicate, clean, and stably sort relation lists."""
...
def extract_training_example(row: Dict[str, Any], system_prompt: Optional[str] = None) -> Dict[str, Any]:
    """Extract the training fields and gold relations from one ChatML row."""
...
def summarize_chat_dataset(path: Path) -> Dict[str, Any]:
    """Summarize dataset size, empty targets, and label distribution."""
...
def _safe_percentile(values: Sequence[int], percentile: float) -> int:
    """Compute a simple percentile without extra numeric dependencies."""
    if not values:
        return 0
...
def _summarize_numeric(values: Sequence[int]) -> Dict[str, float]:
    """Compress numeric values into a standard summary dictionary."""
    if not values:
        return {
            "min": 0,
            "max": 0,
...
def compute_dataset_statistics(
    ...
    *,
    ...
    When a tokenizer is provided, this also records prompt length, full-example
    length, and whether each example exceeds `max_length`.
    """
    rows = read_jsonl(path)
    ...
    for row in rows:
        ...
        if tokenizer is not None:
            # Track both prompt-only and prompt-plus-target lengths.
...
def tokenize_supervised_example(
    ...
    *,
    ...
    """Encode one supervised example into training inputs.

    Returns `None` when the example cannot be used safely, most commonly
    because it exceeds `max_length`, the assistant span cannot be located
    reliably, or every label token is masked out.
    """
    ...
    if supports_assistant_tokens_mask(tokenizer):
        try:
            # Prefer the native assistant mask path when tokenizer support exists.
            ...
            # Some tokenizers expose the interface but remain unstable at runtime.
            pass

    # Fallback: encode prompt and full messages separately, then derive labels from the length difference.
    prompt_ids = apply_chat_template(
        tokenizer,
        prompt_messages,
        tokenize=True,
...
class DatasetBuildStats:
    """Core counters recorded during dataset construction."""
    num_rows: int = 0
    num_encoded: int = 0
...
class SupervisedChatDataset(Dataset):
    """Simple wrapper around encoded samples for Hugging Face Trainer."""
...
class SupervisedDataCollator:
    ...
    In addition to `input_ids`, `attention_mask`, and `labels`, this collator
    also keeps per-sample `loss_weights` for custom loss computation.
    """
    ...
    def __call__(self, features: List[Dict[str, List[int]]]) -> Dict[str, torch.Tensor]:
        """Align a batch of variable-length samples to a shared length."""
...
def compute_relation_weight(
    ...
    *,
    ...
    """Heuristically compute weights from relation type and example difficulty.

    The same logic is used for both sampling weights and per-example loss
    weights.
    """
    weight = 1.0
    if not relations:
...
def build_supervised_dataset(
    ...
    *,
    ...
    This function loads rows, extracts training fields, tokenizes them, applies
    length filtering, and computes sampling and loss weights.
    """
    rows = read_jsonl(path)
    if limit is not None:
        rows = rows[:limit]
