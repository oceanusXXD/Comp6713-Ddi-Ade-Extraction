"""Inference config loading and CLI override helpers."""
...
def split_to_default_path(split: str) -> Path:
    """Map split aliases to the repository default dataset paths."""
...
def load_inference_config(config_path: str | Path, *, validate: bool = True) -> Dict[str, Any]:
    """Load the inference config and resolve paths, model sources, and outputs."""
...
def validate_inference_config(config: Dict[str, Any]) -> None:
    """Validate key inference settings before execution."""
...
def apply_cli_overrides(config: Dict[str, Any], args: Any) -> Dict[str, Any]:
    ...
    if args.input_text is not None:
        # Single-text mode should not require a dataset file.
        merged["data"]["input_path"] = None
