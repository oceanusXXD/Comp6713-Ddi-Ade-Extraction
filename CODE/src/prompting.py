"""Prompt loading and chat-template helper utilities."""
...
def extract_message_content(messages: List[Dict[str, Any]], role: str) -> str:
    """Return the first message content for the requested role."""
...
def load_system_prompt(system_prompt_path: Optional[str] = None) -> str:
    """Load an external prompt file when provided, otherwise use the built-in default."""
...
def build_messages(system_prompt: str, user_text: str, assistant_text: Optional[str] = None) -> List[Dict[str, str]]:
    """Build a standard ChatML-style message list."""
    messages = [
...
def supports_chat_template(tokenizer: Any) -> bool:
    """Return whether the tokenizer exposes the HF chat-template interface."""
    return callable(getattr(tokenizer, "apply_chat_template", None))


def supports_assistant_tokens_mask(tokenizer: Any) -> bool:
    """Return whether the tokenizer chat template can emit an assistant mask."""
...
def should_try_enable_thinking(tokenizer: Any, enable_thinking: Optional[bool]) -> bool:
    """Return whether it is worth forwarding `enable_thinking` to the tokenizer."""
...
def apply_chat_template(
    ...
    *,
    ...
    """Wrap `tokenizer.apply_chat_template` behind a common compatibility layer."""
