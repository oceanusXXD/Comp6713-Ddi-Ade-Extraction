...
Metric: **Micro Precision / Recall / F1** with triple-level exact match (`relation_type` + `head_entity` + `tail_entity`, case-insensitive).

## Results

| Dataset | Type | Base P | Base R | Base F1 | LoRA P | LoRA R | LoRA F1 | Delta F1 |
|---------|------|-------:|-------:|--------:|-------:|-------:|--------:|---------:|
...
- The base model occasionally fails to output valid JSON on some datasets.
