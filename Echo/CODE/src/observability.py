...
"""Helpers for writing reproducibility and observability artifacts."""
...
def json_default(value: Any) -> Any:
    """Provide JSON serialization support for ``Path`` and ``torch.dtype`` values."""
...
def collect_runtime_environment() -> Dict[str, Any]:
    """Collect Python, CUDA, and package-origin information for the current run."""
...
def collect_parameter_statistics(model: Any) -> Dict[str, Any]:
    """Summarize total and trainable parameter counts for a model."""
    total_parameters = 0
...
class JsonlMetricsCallback(TrainerCallback):
    """Mirror Trainer log events into a JSONL file."""

    def __init__(self, output_path: Path):
        self.output_path = output_path

    def on_log(self, args, state, control, logs=None, **kwargs):
        """Append one record whenever the Trainer emits a log event."""
        if not logs:
            return
        payload = {
