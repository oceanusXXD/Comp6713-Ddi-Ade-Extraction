"""Prompt-loading and chat-template helpers."""
...
def extract_message_content(messages: List[Dict[str, Any]], role: str) -> str:
    """Return the first message content matching the requested role."""
...
def load_system_prompt(system_prompt_path: Optional[str] = None) -> str:
    """Load an external prompt file or fall back to the packaged default prompt."""
...
def supports_chat_template(tokenizer: Any) -> bool:
    """Return whether the tokenizer exposes the Hugging Face chat-template API."""
...
def should_try_enable_thinking(tokenizer: Any, enable_thinking: Optional[bool]) -> bool:
    """Return whether ``enable_thinking`` is worth forwarding to the tokenizer."""
...
def apply_chat_template(
    ...
    *,
    ...
    """Wrap ``tokenizer.apply_chat_template`` with compatibility handling."""
